...
function refresh(){
    if(localnotes.length==0) noteList.innerHTML=`<p style="color:rgb(150,150,150)">No notes yet. Click on 'Add Note' to create a new one...<p>`;
    else{
        noteList.innerHTML=""
        localnotes.forEach((element,index) => {
            ...
            <div class="card-body">
                ...
                <button class="btn btn-primary delBtn" id="delBtn${index+1}" onclick="delnote(this.id)">Delete Note</button>
            </div>
        </div>`
        });
    }
...
searchbar.addEventListener("input", function(){
        noteList.innerHTML=""
        localnotes.forEach((element,index) => {
            if(element.value.toLowerCase().includes(searchbar.value.toLowerCase()) ||
        ...
        if(noteList.innerHTML=="") noteList.innerHTML=`<p style="color:rgb(150,150,150)">No note matches with your search keyword...<p>`
